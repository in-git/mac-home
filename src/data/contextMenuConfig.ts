...
/**
 * 右键菜单配置 —— 将「桌面空白处」与「应用」的右键菜单拆分成两个独立、可配置的列表。
 *
 * 之前两类菜单的条目散落在 ContextMenu.tsx 里硬编码。现在统一用下面的配置数组驱动，
 * 修改菜单项只需编辑此处，无需改动渲染应用。
 *
 * 配置项字段说明：
 *  - id:      唯一标识
 *  - label:   显示文案
 *  - icon:    lucide 图标应用
 *  - action:  触发的动作（应用根据 action 映射到对应的处理函数）
 *  - danger:  是否为危险操作（红色样式，用于「移除该应用」）
...
/**
 * 应用（widget）右键菜单配置。
 * 注意：应用标题与「调整尺寸」区为应用专属动态内容，由 ContextMenu 在渲染时统一附加，
 * 此处仅配置尺寸区下方的操作型菜单项。
 */
export const WIDGET_CONTEXT_MENU: ContextMenuItemConfig[] = [
  ...
  },
  {
    ...
    icon: Pencil,
    action: 'editIcon',
    // 仅对 icon / web-app / settings 应用显示（应用内由 resolveAction 根据类型控制）
    dividerAfter: true,
  },
  {
    ...
    // 仅当该应用类型在「二级配置子菜单注册表」中注册了子菜单时才显示
