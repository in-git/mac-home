...
import { Plus, Pin, Trash2, CheckSquare, Square, Palette, Calendar } from 'lucide-react';
import { StickyNote as StickyNoteType, NoteColor } from '../types';
...
export const StickyNotesWidget: React.FC<Props> = ({ notes, onUpdateNotes }) => {
  const [activeNoteId, setActiveNoteId] = useState<string>(notes[0]?.id || '');

  const activeNote = notes.find((n) => n.id === activeNoteId) || notes[0];

  const colorStyles: Record<NoteColor, { bg: string; text: string; accent: string }> = {
    yellow: {
    ...
    <div className="h-full flex flex-col justify-between text-xs p-1">
      ...
      {activeNote ? (
        ...
        >
            ...
            <div className="flex items-center space-x-1 shrink-0 ml-2">
              ...
              <button
                ...
                className={`p-1 rounded-md transition-colors ${
                  activeNote.pinned ? 'bg-black/10 dark:bg-white/20' : 'hover:bg-black/5'
                }`}
                title="固定便签"
              >
              ...
              <div className="flex items-center space-x-1 px-1">
                {(['yellow', 'mint', 'pink', 'lavender', 'blue', 'glass'] as NoteColor[]).map((c) => (
                  <button
                    key={c}
                    onClick={() => {
                      playSound.playClick();
            ...
            {activeNote.isChecklist && activeNote.checklistItems ? (
              <div className="space-y-1.5 pt-1">
                {activeNote.checklistItems.map((item) => (
                  ...
                  >
                    {item.completed ? (
                      <CheckSquare size={14} className="text-[#007AFF] shrink-0" />
                    ) : (
                      <Square size={14} className="text-slate-400 shrink-0" />
                    )}
                    <span
                      className={`text-xs ${
                        item.completed ? 'line-through opacity-50' : 'font-medium'
                      }`}
                    >
                      {item.text}
                    </span>
          ...
          {/* Footer timestamp */}
          <div className="mt-2 pt-2 border-t border-black/5 dark:border-white/10 flex items-center justify-between text-[10px] opacity-60">
        ...
        <div className="text-center py-8 text-slate-400">暂无便签，点击右上角新建</div>
      )}
    </div>
  );
};
