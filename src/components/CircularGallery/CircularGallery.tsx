...
class App {
  ...
  constructor(
    ...
    // Align to the requested initial item without an opening animation.
    if (initialIndex != null && this.medias[0]) {
      const offset = initialIndex * this.medias[0].width;
      this.scroll.current = this.scroll.target = offset;
  ...
  /** Jump to a specific item index, choosing the shortest looping direction. */
  goTo(index: number) {
    if (!this.medias[0] || !this.itemCount) return;
    const width = this.medias[0].width;
    const n = this.itemCount;
    const currentRaw = Math.round(this.scroll.current / width);
    const currentIndex = ((currentRaw % n) + n) % n;
    let delta = index - currentIndex;
    if (delta > n / 2) delta -= n;
    if (delta < -n / 2) delta += n;
    this.scroll.target = this.scroll.current + delta * width;
  }
  ...
  update() {
    ...
    // Report the currently centered item so callers can sync UI / selection.
    if (this.itemCount && this.medias[0]) {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      ...
      {showControls && count > 1 && (
        <>
          <button
            ...
            className="absolute left-4 top-1/2 z-10 grid h-[42px] w-[42px] -translate-y-1/2 place-items-center rounded-full border border-white/20 bg-[rgba(18,20,26,0.55)] text-white backdrop-blur-md transition hover:border-white/40 hover:bg-[rgba(28,31,40,0.85)] active:scale-95"
          >
          ...
          <button
            ...
            className="absolute right-4 top-1/2 z-10 grid h-[42px] w-[42px] -translate-y-1/2 place-items-center rounded-full border border-white/20 bg-[rgba(18,20,26,0.55)] text-white backdrop-blur-md transition hover:border-white/40 hover:bg-[rgba(28,31,40,0.85)] active:scale-95"
          >
            <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
      ...
      {showIndicators && count > 1 && (
        <div className="absolute bottom-4 left-1/2 z-10 flex -translate-x-1/2 gap-2 rounded-full bg-[rgba(14,16,22,0.4)] px-3 py-2 backdrop-blur-sm">
