...
const DepthCarousel = ({
  ...
  const layout = useCallback((pos: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // 叠加单项自定义滤镜（如桌面主题预设），让最前方卡片呈现真实的选中效果
      const itemFilter = data[i].filter ? ` ${data[i].filter}` : '';

      el.style.transform = `translate(-50%, -50%) scale(${sc}) translateX(${tx.toFixed(2)}px) translateZ(${tz.toFixed(2)}px) rotateY(${ry.toFixed(3)}deg)`;
      el.style.opacity = opacity.toFixed(3);
      el.style.filter = `brightness(${brightness.toFixed(3)}) blur(${blurPx.toFixed(2)}px)${itemFilter}`;
  ...
  // 挂载时定位到 initialIndex（若指定），用于对齐当前已选中的主题卡片
  useEffect(() => {
    if (initialIndex == null || count === 0) return;
    setFocus(initialIndex, false);
    // 仅在挂载时执行一次
