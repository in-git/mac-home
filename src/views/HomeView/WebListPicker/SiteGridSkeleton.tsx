...
/** 超大卡骨架：与 SiteHeroCard 同比例（2:1），底部压标题与描述的浮层 */
const HeroSkeleton: React.FC = () => (
  ...
  >
    ...
    <div className="absolute inset-x-0 bottom-0 flex flex-col gap-2 bg-gradient-to-t from-black/30 to-transparent p-3 sm:p-4">
...
/** 排行榜卡骨架：顶部 tabbar 两条 + 5 行榜单，行结构与真实卡片一致 */
const RankSkeleton: React.FC = () => (
  ...
  >
    {/* tabbar 占位 */}
    <div className="flex shrink-0 items-center gap-4 border-b border-black/5 px-3 py-2.5 sm:gap-5 sm:px-3.5 sm:py-3 dark:border-white/10">
      <Line className="h-4 w-8 sm:h-5 sm:w-10" />
      <Line className="h-4 w-8 sm:h-5 sm:w-10" />
    </div>
    {/* 榜单行占位：与 RankRow 同为 flex-1 均分，行高随卡片高度自适应 */}
    <div className="flex min-h-0 flex-1 flex-col">
      {Array.from({ length: 5 }).map((_, idx) => (
        <div key={idx} className="flex min-h-0 flex-1 items-center gap-2 px-3">
          ...
          <Block className="h-4 w-4 shrink-0 rounded-full sm:h-[18px] sm:w-[18px]" />
...
export const SiteGridSkeleton: React.FC = () => (
