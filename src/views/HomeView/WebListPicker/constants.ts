 ...
 * 宫格内**末张**推荐站点卡的显隐控制。
 *
 * 宫格内部无论哪一档都是 2 列（见 FEATURED_TILES_CLASS），差别只在外层跨列数，
 * 所以这里的判断依据是「宫格自身占几列」而非整屏宽度：
 * - < lg（宫格跨满整行）：排行榜卡跨 2 行独占左列，右列只有 2 格，
 *   第 3 张会溢出到第 3 行且左侧空一格，视觉上「多出来一张」，故隐藏；
 * - lg 起（宫格只占 1 列）：排行榜卡只占 1 格，3 张正好铺满其余 3 格，需要显示。
 *
 * 注意断点是 `lg`（与宫格并排的临界点一致）：sm ~ lg 宫格虽只跨 1 列，
 * 但仍是「排行榜卡跨 2 行 + 右列 2 格」的堆叠结构，同样只放 2 张。
 */
